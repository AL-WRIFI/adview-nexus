...
interface RelatedAdsCarouselProps {
  ads: Listing[];
  categoryId?: number;
  title?: string;
}

export function RelatedAdsCarousel({ 
  ads, 
  categoryId,
  title = "إعلانات مشابهة"
}: RelatedAdsCarouselProps) {
    ...
    <div className="mt-12 border-t border-neutral-200 dark:border-neutral-700 pt-8">
      ...
      >
        <CarouselContent className="-ml-2 md:-ml-4">
          {ads.map((ad) => (
            <CarouselItem key={ad.id} className="pl-2 md:pl-4 md:basis-1/3 lg:basis-1/4 xl:basis-1/5">
