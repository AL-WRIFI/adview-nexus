...
interface ClassifiedFiltersUIProps {
  onFilterChange: (filters: SearchFilters) => void;
  onSearch: (query: string) => void;
}

// Mock data for comprehensive filtering
...
export function ClassifiedFiltersUI({ onFilterChange, onSearch }: ClassifiedFiltersUIProps) {
  ...
  useEffect(() => {
    if (selectedCategory && [1,2,3,4,5,6].includes(selectedCategory)) {
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {animalSubCategories.map((animal) => (
            <button
              ...
              className={cn(
                "p-4 text-center transition-all hover:bg-gray-100",
                selectedSubCategory === animal.id && "bg-blue-50 border-2 border-blue-500"
              )}
            >
        ...
        <div className="grid grid-cols-3 gap-3">
          {animalCategories.map((category) => (
            <button
              ...
              className={cn(
                "p-4 text-center transition-all hover:bg-gray-100 border rounded-lg",
                selectedCategory === category.id && "bg-blue-50 border-2 border-blue-500"
              )}
            >
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <button
            ...
            className="p-4 text-center transition-all hover:bg-gray-100"
          >
          ...
          <div className="grid grid-cols-3 gap-3">
            {selectedCategoryData.subcategories.map((subcat) => (
              <button
                ...
                className={cn(
                  "p-3 text-center text-sm transition-all hover:bg-gray-100 border rounded-lg",
                  selectedSubCategory === subcat.id && "bg-blue-50 border-2 border-blue-500"
                )}
              >
                {subcat.name}
              </button>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {displayedBrands.map((brand) => (
            <button
              ...
              className={cn(
                "p-4 transition-all hover:bg-gray-100",
                selectedBrand === brand.id && "bg-blue-50 border-2 border-blue-500"
              )}
            >
      ...
      {(selectedCategory || selectedSubCategory || selectedBrand || searchQuery) && (
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {selectedBrand && (
              <Badge variant="secondary" className="px-3 py-1">
                {currentBrands.find(b => b.id === selectedBrand)?.name}
              </Badge>
            )}
          </div>
        </div>
